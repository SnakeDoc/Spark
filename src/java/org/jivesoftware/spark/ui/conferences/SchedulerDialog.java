 ...
 * $Date: $
 *
 * Copyright (C) 2004-2011 Jive Software, 2014 Ignite Realtime. All rights reserved.
 *
...
final class SchedulerDialog extends JPanel {
    ...
    public SchedulerDialog(boolean adhoc) {
        if (adhoc) {
        ...
        add(new JLabel("Date and Time: "), new GridBagConstraints(0, 4, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, new Insets(15, 5, 5, 5), 0, 0));
        add(buildDateTimePanel(new Date()), new GridBagConstraints(1, 4, 3, 1, 1.0, 1.0, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
    ...
    private JPanel buildDateTimePanel(final Date value) {
    	JPanel datePanel = new JPanel();

    	JXDatePicker dateChooser = new JXDatePicker();
    	if (value != null) {
    	    dateChooser.setDate(value);
    	}
    	for (Component comp : dateChooser.getComponents()) {
    	    if (comp instanceof JTextField) {
    		((JTextField) comp).setColumns(24);
    	...
    	JComponent editor = new JSpinner.DateEditor(timeSpinner, 
    					" " + SettingsManager.getLocalPreferences().getTimeFormat());
    ...
    public void inviteUsersToRoom(final String serviceName, Collection<BookmarkedConference> rooms, String adHocRoomName, Collection<String> jids) {
        ...
        titlePanel = new TitlePanel(Res.getString(/*"title.invite.to.conference"*/"Schedule a conference"), Res.getString(/*"message.invite.users.to.conference"*/"Setup a scheduled conference room"), SparkRes.getImageIcon(SparkRes.BLANK_IMAGE), true);
